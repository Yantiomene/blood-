...
const RequestDetailPage: React.FC = () => {
  ...
  //     try {
  //       setShareMsg('');
  //       const url = typeof window !== 'undefined' ? `${window.location.origin}/dashboard/donor-requests/${params.id}` : '';
  //       const title = `Blood+ Request #${params.id}`;
  //       const text = `Need ${request?.quantity} ml of ${request?.bloodType}. Can you help?`;
  ...
  const onShare = async () => {
    ...
    try {
      setShareMsg('');
      const url = typeof window !== 'undefined' ? `${window.location.origin}/dashboard/donor-requests/${detail.id}` : '';
      const title = `Blood+ Request #${detail.id}`;
      const text = `Need ${detail.quantity} ml of ${detail.bloodType}. Can you help?`;
      if ((navigator as any).share) {
        await (navigator as any).share({ title, text, url });
        setShareMsg('Thanks for sharing!');
      } else {
        ...
        {detail && (
          <div className="bg-white rounded shadow p-4">
            <div className="flex items-center justify-between mb-2">
              <h1 className="text-xl font-bold">Request #{detail.id}</h1>
              <div className="flex items-center gap-2">
                <button onClick={onShare} className="px-3 py-1 rounded bg-slate-200 hover:bg-slate-300 text-slate-800">Share</button>

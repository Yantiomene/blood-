...
const buttonStyle =
  "block text-slate-600 border border-slate-300 hover:text-red-600 px-3 py-2 hover:bg-slate-200 active:bg-slate-300 rounded-md transition duration-100";
...
const Dashboard = () => {
  ...
  return (
    <>
      <header className="bg-slate-200 py-4">
        <nav className="container mx-auto px-4 py-2 flex items-center justify-between">
          <div className="flex items-center gap-x-2">
            ...
            {userData.isDonor && (
              <span className="px-3 py-1 rounded-full text-xs bg-green-400 text-white italic">
                proud donor
              </span>
            )}
          </div>
      ...
      {showCreateRequest && (
        <Overlay showWindow={setShowCreateRequest}>
          <DonationRequestForm />
        </Overlay>
      )}
      <div className="container md:w-[60vw] md:m-auto mx-auto px-4 py-8">
        <nav className="flex justify-between items-center relative">
          ...
          <ul className="flex flex-col w-fit bg-white overflow-hidden rounded-md shadow-md absolute top-0 right-0">
            ...
            >
              Filter by date {showDateFilterList ? "↓" : "↑"}
            </span>
            {showDateFilterList && (
        ...
        <small className="text-sm italic text-gray-400 ">
          Displaying ({`${requestList.length}`}) blood donation requests{" "}
        </small>
        ...
        <div className="bg-slate-200 h-[600px] my-10 p-4 rounded">
          <h1 className="mt-20 text-5xl text-center text-slate-400 font-bold">
            say something
          </h1>
          {/* <Loader size="50px" /> */}
        </div>
